...
/* if is is, add the distance to downstream patch */
		...
		if ( (flow_table[pch].land == 1) )
			...
			for (neigh = 1; neigh <= flow_table[pch].num_adjacent; neigh ++)
				...
				if (aptr->gamma > 0.0) {	
					xrun = (float)( pow( (flow_table[pch].x - flow_table[inx].x), 2.0) );
					yrun = (float)( pow( (flow_table[pch].y - flow_table[inx].y), 2.0) );
			...
			if (n_adjacent == 0) {
				...
				drainage_density += ( float )( sqrt(flow_table[pch].area) * (cell) );
				}
			}	
