...
double find_top(struct flow_struct *flow_table, int curr, double pit_elev,
		int *num_pit, int *upslope_list, int *edge_inx)

{
	...
	if (flow_table[curr].num_adjacent > 0) {
		...
		if ((*edge_inx == 0)
				|| (flow_table[curr].z < flow_table[*edge_inx].z)) {
			...
			for (i = 1; i <= flow_table[curr].num_adjacent; i++) {
				...
				if ((aptr->gamma > 0) && (new == 0)) {
					...
					if (((top_elev < min_elev) || (min_elev == 0.0))
							&& (top_elev > 0.0) && (aptr->z < pit_elev)) {
						*edge_inx = aptr->inx;
						min_elev = top_elev;
					}
				}

				else if ((aptr->gamma == 0.0) && (new == 0)) {
					*num_pit += 1;
					upslope_list[*num_pit] = inx;
					next_edge_inx = *edge_inx;
					top_elev = find_top(flow_table, inx, pit_elev, num_pit,
							upslope_list, &(next_edge_inx));
					if (((top_elev < min_elev) || (min_elev == 0.0))
							&& (top_elev > 0.0)) {
