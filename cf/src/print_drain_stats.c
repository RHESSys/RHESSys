...
void print_drain_stats(num_patches, flow_table, input_prefix)
	struct flow_struct *flow_table;int num_patches;char *input_prefix;

{
	...
	for (i = 1; i <= num_patches; i++) {
		...
		fprintf(outfile,
				...
				flow_table[i].z, flow_table[i].acc_area, flow_table[i].land,
				flow_table[i].total_gamma, flow_table[i].slope,
				flow_table[i].area, flow_table[i].num_adjacent);
