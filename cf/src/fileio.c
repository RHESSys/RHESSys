...
output_char(array, filename, maxr, maxc)
	char array[];int maxr, maxc;char filename[]; {
	FILE *out1, *fopen();
...
output_short2(array, filename, maxr, maxc)
	short int array[];int maxr, maxc;char filename[]; {
	FILE *out1, *fopen();
...
output_float(array, filename, maxr, maxc)
	int maxr, maxc;float array[];char filename[]; {
	FILE *out1, *fopen();
...
output_int2(array, filename, maxr, maxc)
	int maxr, maxc;int array[];char filename[]; {
	FILE *out1, *fopen();
...
void input_short2(array, filename, maxr, maxc)
	int maxr, maxc;short int array[];char filename[]; {
	FILE *in1;
...
void input_ascii_int(array, filename, mc, mr, arc_flag)
	int *array;int mc, mr, arc_flag;char *filename; {
	FILE *in1, *fopen();
	int r;
	int max;
...
void input_ascii_sint(array, filename, mc, mr, arc_flag)
	short int *array;int mc, mr, arc_flag;char *filename; {
	FILE *in1, *fopen();
...
void input_ascii_float(float *array, char *filename, int mc, int mr,
		int arc_flag, float sc)
...
void output_ascii_double(double *array, char *filename, int mc, int mr) {
	...
	if ((in1 = fopen(filename, "w")) == NULL ) {
		...
		for (r = 0; r < mr * mc; r++) {
			...
			if (new > max)
				max = new;
			fprintf(in1, "%g ", array[r]);
		}

		printf("\n Max for ew horizon is %g", max);
		fclose(in1);
...
void input_int2(array, filename, mc, mr)
	int *array;int mc, mr;char *filename; {
	FILE *in1, *fopen();
...
void input_float2(array, filename, mr, mc)
	float *array;int mc, mr;char filename[];
...
input_short_ASCII(array, filename, maxr, maxc)
	int maxr, maxc;short int array[][MAXCOL];char filename[]; {
	FILE *in1, *fopen();
	int r, c;
	short int elev;
...
input_header(maxr, maxc, fndem, arc_flag)
	int *maxr, *maxc;int arc_flag;char *fndem; {
	FILE *in1, *fopen();
	char tmp[100];
	int i;
...
void input_prompt(maxr, maxc, filename, fndem, fnslope, fnflna, fnpatch, fnzone,
		fnhill, fnstream, fnroads, fnsewers, fnpartition, fntable, fnroot,
		fnehr, fnwhr, f_flag, sewer_flag, arc_flag)
	int *maxr, *maxc;char *filename;char *fndem, *fnpatch, *fnzone, *fnhill,
			*fnstream, *fnroads, *fnpartition, *fntable, *fnroot, *fnflna,
			*fnslope, *fnsewers, *fnehr, *fnwhr;int f_flag, arc_flag,
			sewer_flag;
...
void copy_short_m2(m1, num, m2, maxr, maxc)
	int maxr, maxc;short int m1[], num, m2[]; {
	int r, c;
...
copy_int_m_char2(m1, num, m2, maxr, maxc)
	int maxr, maxc;int m1[], num;char m2[]; {
	int r, c;
...
copy_short_m_char2(m1, num, m2, maxr, maxc)
	int maxr, maxc;short m1[], num;char m2[]; {
	int r, c;
