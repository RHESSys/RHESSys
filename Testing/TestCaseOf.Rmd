...
rhessys_ver = "/Users/naomitague/Model/RHESSys/rhessys/rhessys7.5"
...
cor.test(test$obs_m, test$model_Q)

# error/bias
test_err = (test$model_Q - test$obs_m) %>% mean(na.rm=TRUE)
base_err = (base$model_Q - base$obs_m) %>% mean(na.rm=TRUE)

test_err/mean(test$obs_m, na.rm=TRUE)*100
