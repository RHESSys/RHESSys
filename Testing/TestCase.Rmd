...
we also estimate max and min percent differences
...
# baseline 
tmp = base$bd %>% select(date, streamflow, lai, snowpack, psn, unfilled_cap, trans, evap, height)
tmp2 = base$bdg %>% select(plantc, streamflow_NO3, litrc, soilc, gpsn, plant_resp, nitrif, denitrif)
tmpb = cbind.data.frame(tmp, tmp2)
tmpb$new =FALSE
# new results
tmp = test$bd %>% select(date, streamflow, lai, snowpack, psn, unfilled_cap, trans, evap, height)
...
i = ggplot(tmpa, aes(date, unfilled_cap, col=new))+geom_line()
