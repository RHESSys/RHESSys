...
GIS_read = function(type,readin,typepars) {
  ...
  if (length(readin)==1 & (startsWith(basename(readin),"template.") | startsWith(basename(readin),"Template.") | endsWith(basename(readin),".template") ) ) { # if starts or ends with "template"
    template = readin
