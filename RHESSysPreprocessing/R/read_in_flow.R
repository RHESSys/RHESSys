...
read_in_flow<-function(input_file){
  num_patches<-scan(file=input_file,nlines=1)  #read in number of patches
  ...
  for (p_i in 1:num_patches){       #p_i is which patch we are on
    y<-scan(file=input_file,nlines=1,skip=line_count) #read in a line of patch data, 11th num is munber of neighbors
    line_count<-line_count+1      #move to next line
    
    neighbor_list<-list()
    for (j in 1:y[11]) {      #read in neighbor data
      n_y<-scan(file=input_file,nlines=1,skip=line_count)  #read in neighbor data, patchID,zoneID,hillID, proportion of flow
      line_count<-line_count+1      #move to next line
