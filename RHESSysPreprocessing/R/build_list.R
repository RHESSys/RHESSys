...
build_list<-function(flw_struct,border_count,cell_length){
  num_rows<-length(flw_struct$Number)
  neighbors<-list()
  for (i in 1:num_rows){
    neighbor_index<-which((border_count[i,]>0)) #find neighbors
    neighbor_index<-neighbor_index[-which(neighbor_index==i)] #remove self from neigbor list
    
    tp_perimeter<-cell_length*border_count[i,neighbor_index]
