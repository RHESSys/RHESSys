...
multiscale_flow = function(CF1, map_ar_clean, cfmaps, asp_list){
  ...
  for (p in raw_patch_data[!is.na(raw_patch_data)]) {
    ...
    for (asp in 1:asp_count) {
      ...
      CF2[[length(CF2)]]$Area = CF1[[which(patch_ID == p)]]$Area * as.numeric(rulevars[[id]]$patch_level_vars[rulevars[[id]]$patch_level_vars[,1] == "pct_family_area",asp + 1]) # change area
      ...
      old_boarder = CF1[[which(patch_ID == p)]]$Border_perimeter
      ...
      for (nbr in old_nbrs) { # loop through old neighbors - neighbors are numbers not patches
        ...
        for (nbr_asp in 1:nbr_asp_ct) { # for each asp for each neighbor
          ...
          new_gammas = c(new_gammas, gamma * as.numeric(rulevars[[nbr_id]]$patch_level_vars[rulevars[[nbr_id]]$patch_level_vars[,1] == "pct_family_area",nbr_asp + 1]))
        }
      }
