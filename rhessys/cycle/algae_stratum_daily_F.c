							   ...
							   struct 	date                    current_date)
{
    ...
    double max_nitlimited_growth, growth, death, respire, settle, lost;
    double growth_rate, respire_rate, settling_rate;
    double Kdown_direct, Kdown_diffuse;
    double Kup_diffuse, Kup_direct;
    ...
    if(patch[0].detention_store > 0){
    	...
    	/*--------------------------------------------------------------*/
        /*	I. Seed chla if required                                    */
    	/*--------------------------------------------------------------*/
  		/*  If preday detention store = 0, and today > 0, seed chlA */ 
         if(patch[0].preday_detention_store <= 0 || stratum[0].algae.preday_chla <= stratum[0].defaults[0][0].algae.chla_resprout_conc * patch[0].area * patch[0].detention_store/1000){
             ...
             stratum[0].algae.resprouted_N_cum += stratum[0].algae.chla * 1/stratum[0].defaults[0][0].algae.chla_to_N;
             stratum[0].algae.resprouted_C_cum += stratum[0].algae.chla * 1/stratum[0].defaults[0][0].algae.chla_to_C;
         } 
		...
		/* 1.2 - Light Effects: */           
             ...
             light_extinct = stratum[0].defaults[0][0].algae.light_extinct + stratum[0].defaults[0][0].algae.light_coef_A * (chlaconc*1000*1000) + stratum[0].defaults[0][0].algae.light_coef_B * pow((chlaconc*1000*1000),(0.66667));
         ...
         /*--------------------------------------------------------------*/
         respire_rate =stratum[0].defaults[0][0].algae.respire_rate * pow(stratum[0].defaults[0][0].algae.respire_temp_theta, (patch[0].scm_temp-stratum[0].defaults[0][0].algae.respire_temp_baseT)); 
		 ...
		 death_rate = stratum[0].defaults[0][0].algae.death_rate *pow(stratum[0].defaults[0][0].algae.death_temp_theta, (patch[0].scm_temp-stratum[0].defaults[0][0].algae.death_temp_baseT));
         
         /*--------------------------------------------------------------*/
         /*	4 - Compute Settling                                      */
         /*--------------------------------------------------------------*/  
         settle = stratum[0].algae.chla * min((stratum[0].defaults[0][0].algae.chla_settling_rate/patch[0].scm_ave_height),1); // kg/m2/d
		...
		/*--------------------------------------------------------------*/
		/* Check that not more algae is lost than exists				*/
		/* And if so, partition losses proportionately:					*/
		/*--------------------------------------------------------------*/
         if( (respire+death+settle) > stratum[0].algae.chla) {
			lost = respire + death + settle;
			respire = (respire/lost) * stratum[0].algae.chla;
			death = (death/lost) * stratum[0].algae.chla;
			settle = (settle/lost) * stratum[0].algae.chla;		
		 }
         ...
         /*--------------------------------------------------------------*/
		 /* 5.1 - ALGAE  */
         stratum[0].algae.chla            += growth - respire - death - settle;
         stratum[0].algae.chla			   = max(0,stratum[0].algae.chla);
         stratum[0].algae.chla_settle      = settle;
         stratum[0].algae.chla_settle_cum += settle;
     
        
         /* 5.2 - CARBON  */
         stratum[0].algae.totalC       += (growth - respire - death - settle) / stratum[0].defaults[0][0].algae.chla_to_C;
         ...
         stratum[0].algae.Crelease_cum += death / stratum[0].defaults[0][0].algae.chla_to_C;
         stratum[0].algae.Crelease     = death / stratum[0].defaults[0][0].algae.chla_to_C;
         patch[0].surface_DOC          += death / stratum[0].defaults[0][0].algae.chla_to_C;
         
          /* 5.3 - NITROGEN  */       
         stratum[0].algae.totalN       += (growth - respire - death - settle) / stratum[0].defaults[0][0].algae.chla_to_N;
         ...
         if (patch[0].surface_NH4 >= stratum[0].algae.Nuptake*npref && patch[0].surface_NO3 >= stratum[0].algae.Nuptake*(1-npref)){
            /*if(patch[0].ID==2){
         	fprintf(stderr, "\n BOTH AMPLE (%f): amon: %f | nit: %f | sum: %f | Nuptake: %f = %f + %f | Nrelease: %f | Nrespire: %f", npref, patch[0].surface_NH4, patch[0].surface_NO3, patch[0].surface_NO3+patch[0].surface_NH4, stratum[0].algae.Nuptake, stratum[0].algae.Nuptake*npref, stratum[0].algae.Nuptake*(1-npref), stratum[0].algae.Nrelease, stratum[0].algae.Nrespire);
        	}*/
             ...
             patch[0].surface_NH4  = max(patch[0].surface_NH4,0);
             patch[0].surface_NO3  = max(patch[0].surface_NO3,0);        
         }
         else if (patch[0].surface_NH4 >= stratum[0].algae.Nuptake*npref && patch[0].surface_NO3 < stratum[0].algae.Nuptake*(1-npref)){
            /*if(patch[0].ID==2){
         	fprintf(stderr, "\n AMON AMPLE (%f): amon: %f | nit: %f | sum: %f | Nuptake: %f = %f + %f | Nrelease: %f | Nrespire: %f", npref, patch[0].surface_NH4, patch[0].surface_NO3, patch[0].surface_NO3+patch[0].surface_NH4, stratum[0].algae.Nuptake, (stratum[0].algae.Nuptake - patch[0].surface_NO3), patch[0].surface_NO3, stratum[0].algae.Nrelease, stratum[0].algae.Nrespire);
        	}*/
            ...
            /*f(patch[0].ID==2){
         	fprintf(stderr, "\n NITR AMPLE (%f): amon: %f | nit: %f | sum: %f | Nuptake: %f = %f + %f | Nrelease: %f | Nrespire: %f", npref, patch[0].surface_NH4, patch[0].surface_NO3, patch[0].surface_NO3+patch[0].surface_NH4, stratum[0].algae.Nuptake, patch[0].surface_NH4, (stratum[0].algae.Nuptake - patch[0].surface_NH4),stratum[0].algae.Nrelease, stratum[0].algae.Nrespire);
        	}*/
    ...
    stratum[0].algae.preday_chla = stratum[0].algae.chla;
    stratum[0].algae.Ain = 0;
    stratum[0].algae.Aout = 0;
