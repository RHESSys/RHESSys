...
/*																*/
/*	execute_csv_daily_growth_output_event - output_csv_growths daily data			*/
/*																*/
/*	NAME														*/
/*	execute_csv_daily_growth_output_event - output_csv_growths daily data 	.		*/
/*																*/
...
/*					struct	date	date,  						*/
/*					struct	world_output_csv_growth_file_object 	*outfile)*/
/*																*/
/*	OPTIONS														*/
/*																*/
/*	DESCRIPTION													*/
...
/*	a spatial structure is output_csv_growth only if its appropriate		*/
/*	option flag has been set and its ID matches a 				*/
/*	specified ID, or -999 which indicates all					*/
/*	units at that spatial scale are to be output_csv_growth				*/
/*																*/
/*	PROGRAMMER NOTES											*/
/*																*/
...
/*																*/
/*	March 14, 1997	- 	RAF				*/ 
/*	Allowed output_csv_growth patch to also output_csv_growth the moss strata if	*/
/*		moss is present.				*/
/*--------------------------------------------------------------*/
#include <stdio.h>
#include "rhessys.h"
										  ...
										  struct	world_output_file_object	*outfile)
{
	...
	void output_csv_growth_hillslope(	int,
		struct	hillslope_object *,
		struct	date,
		FILE	*);
	
	void output_csv_growth_zone(	int, int,
		struct	zone_object *,
		struct	date,
		FILE	*);
	...
	if ((command_line[0].b != NULL) || (command_line[0].h != NULL) ||
		...
		for (b=0; b < world[0].num_basin_files; ++ b ) {
			...
			if ((command_line[0].h != NULL) || (command_line[0].z != NULL) ||
				...
				/*--------------------------------------------------------------*/
				/*	output_csv_growth hillslopes 											*/
				/*--------------------------------------------------------------*/
				for (h=0; h < world[0].basins[b][0].num_hillslopes; ++h) {
					/*------------------------------------------------------------*/
					/*	Construct the hillslope output_csv_growth files.						*/
					/*------------------------------------------------------------*/
					...
					if ((command_line[0].z != NULL) || (command_line[0].p != NULL) ||
						(command_line[0].c != NULL)){
						/*---------------------------------------------------------*/
						/*	output_csv_growth zones												*/
						/*--------------------------------------------------------*/
						for(z=0;
						z < world[0].basins[b][0].hillslopes[h][0].num_zones;
						++z) {
							/*------------------------------------------------------*/
							/*	Construct the zone output_csv_growth files.				  */
							/*------------------------------------------------------*/
							...
							if ((command_line[0].p != NULL)
								|| (command_line[0].c != NULL)){
								/*---------------------------------------------*/
								/*	output_csv_growth patches 								  */
								/*---------------------------------------------------*/
								for (p=0;
								p < world[0].basins[b][0].hillslopes[h][0].zones[z][0].num_patches;
								++p) {
									/*-------------------------------------------------*/
									/*	Construct the patch output_csv_growth files.	*/
									/*-----------------------------------------------*/
									if ( command_line[0].p != NULL ){
									...
									/*--------------------------------------------*/
									/*	Construct the canopy_stratum output_csv_growth files*/
									/*------------------------------------------------*/
									if ( command_line[0].c != NULL ){
										/*--------------------------------------------*/
										/*	output_csv_growth canopy stratum 				  */
										/*----------------------------------------------*/
										for (c=0;
		...
		return;
} /*end execute_csv_daily_growth_output_event*/
