...
void execute_monthly_output_event(struct world_object *world,
		struct command_line_object *command_line, struct date date,
		struct world_output_file_object *outfile) {
	/*--------------------------------------------------------------*/
	/*	Local function definition.									*/
	/*--------------------------------------------------------------*/
	void output_monthly_basin(struct basin_object*, struct date, FILE*);

	void output_monthly_hillslope(int, struct hillslope_object*, struct date,
			FILE*);

	void output_monthly_zone(int, int, struct zone_object*, struct date, FILE*);

	void output_monthly_patch(int, int, int, struct patch_object*, struct date,
			FILE*);

	void output_monthly_canopy_stratum(int, int, int, int,
			...
			for (b = 0; b < world[0].num_basin_files; ++b) {
				...
				if (command_line[0].b != NULL) {
					...
					if ((world[0].basins[b][0].ID == basinID)
							|| (basinID == -999))
						output_monthly_basin(world[0].basins[b], date,
							...
							if ((world[0].basins[b][0].ID == basinID)
								...
								if ((world[0].basins[b][0].hillslopes[h][0].ID
									...
									output_monthly_hillslope(
											...
											date, outfile->hillslope->monthly);
						}
						/*------------------------------------------------------------*/
						/*	check to see if there are any lower print options			*/
						/*----------------------------------------------------------*/
