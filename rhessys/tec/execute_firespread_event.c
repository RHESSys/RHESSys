	...
	for (i = 0; i < world[0].num_fire_grid_row; i++)
	{
		for (j = 0; j < world[0].num_fire_grid_col; j++)
		{
			...
			else
			{
				...
				{ // should just be 1 now... - 1 if non MSR, n if MSR where n is patches in patch fam
					if (world[0].defaults[0].fire[0].fire_verbose == 1)
					{
						printf("\nPatch p: %d, i: %d, j:%d\n", p, i, j);
					}
					...
					for (layer = 0; layer < patch[0].num_layers; layer++)
					{
						for (c = 0; c < patch[0].layers[layer].count; c++)
						{
							...
							world[0].fire_grid[i][j].fuel_veg += (
								patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].cover_fraction * 
							...
							if ((patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].defaults[0][0].epc.fire_veg_type == SHRUB))
							{
								if (patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].defaults[0][0].epc.shrub_firespread_flag == 1)
								{
									height_prop = 1.0; // set height proportion to 1 for shrubs with firespread flag
								}
								else if ((patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].epv.height < patch[0].soil_defaults[0][0].understory_height_thresh))
								{
									{
										if (patch[0].soil_defaults[0][0].understory_height_thresh > ZERO)
											height_prop = (patch[0].soil_defaults[0][0].understory_height_thresh -
								...
								world[0].fire_grid[i][j].fuel_litter += height_prop * (patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].cover_fraction * patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].cs.leafc + patch[0].canopy_strata[(patch[0].layers[layer].strata[c])][0].cs.dead_stemc) *
																		patch_fire_grid[i][j].prop_patch_in_grid[p];
							}
