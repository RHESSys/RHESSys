        ...
        struct command_line_object *command_line)
{
	...
	for (i=0 ; i<num_default_files; i++){
		...
		default_object_list[i].load_k1=getDoubleParam(&paramCnt, &paramPtr, "load_k1", "%lf", 3.9, 1);
  		printf("load_k1: %lf\n",default_object_list[i].load_k1);
		default_object_list[i].load_k2=getDoubleParam(&paramCnt, &paramPtr, "load_k2", "%lf", 0.07, 1);
		...
		default_object_list[i].moisture_k2=getDoubleParam(&paramCnt, &paramPtr, "moisture_k2", "%lf", 0.27, 1);
		printf("moisture_k2: %lf\n",default_object_list[i].moisture_k2);
		default_object_list[i].winddir_k1=getDoubleParam(&paramCnt, &paramPtr, "winddir_k1", "%lf", 0.87, 1);
		printf("winddir_k1: %lf\n",default_object_list[i].winddir_k1);
 		default_object_list[i].winddir_k2=getDoubleParam(&paramCnt, &paramPtr, "winddir_k2", "%lf", 0.48, 1);
		printf("winddir_k2: %lf\n",default_object_list[i].winddir_k2);
		default_object_list[i].moisture_ign_k1=getDoubleParam(&paramCnt, &paramPtr, "moisture_ign_k1", "%lf", 3.8, 1);
		printf("moisture_ign_k1: %lf\n",default_object_list[i].moisture_ign_k1);
		default_object_list[i].moisture_ign_k2=getDoubleParam(&paramCnt, &paramPtr, "moisture_ign_k2", "%lf", 0.27, 1);
		printf("moisture_ign_k2: %lf\n",default_object_list[i].moisture_ign_k2);
		default_object_list[i].windmax=getDoubleParam(&paramCnt, &paramPtr, "windmax", "%lf", 1, 1);
		...
		default_object_list[i].spread_calc_type=getIntParam(&paramCnt, &paramPtr, "spread_calc_type", "%d", 9, 1);
		printf("spread_calc_type: %d\n",default_object_list[i].spread_calc_type);
		default_object_list[i].mean_log_wind=getDoubleParam(&paramCnt, &paramPtr, "mean_log_wind", "%lf", 0.494, 1);
		printf("mean_log_wind: %lf\n",default_object_list[i].mean_log_wind);
		default_object_list[i].sd_log_wind=getDoubleParam(&paramCnt, &paramPtr, "sd_log_wind", "%lf", 0.654, 1);
		printf("sd_log_wind: %lf\n",default_object_list[i].sd_log_wind);
		default_object_list[i].mean1_rvm=getDoubleParam(&paramCnt, &paramPtr, "mean1_rvm", "%lf", 1.71, 1);
		printf("mean1_rvm: %lf\n",default_object_list[i].mean1_rvm);
		default_object_list[i].mean2_rvm=getDoubleParam(&paramCnt, &paramPtr, "mean2_rvm", "%lf", -1.91, 1);
		printf("mean2_rvm: %lf\n",default_object_list[i].mean2_rvm);
		default_object_list[i].kappa1_rvm=getDoubleParam(&paramCnt, &paramPtr, "kappa1_rvm", "%lf", 2.37, 1);
