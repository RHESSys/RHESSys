...
/*                      UTM.c                                         */
/*                                                                    */
/*UTM.c - for convert WGS84 coordination system to UTM coordinate sys */
/* author Alex Hajnal                                                 */
/* took for rhessys climate data downscaling in 2019.05.31                                            */
/* reference websit                                                   */
...
int LatLonToUTMXY (FLOAT lat, FLOAT lon, int zone, FLOAT *x_utm, FLOAT *y_utm) {
  if ( (zone < 1) || (zone > 60) )
    zone = FLOOR((lon + 180.0) / 6) + 1;

  MapLatLonToXY (DegToRad(lat), DegToRad(lon), UTMCentralMeridian(zone), x_utm, y_utm);

  /* Adjust easting and northing for UTM system. */
  *x_utm = *x_utm * UTMScaleFactor + 500000.0;
  *y_utm = *y_utm * UTMScaleFactor;
  if (*y_utm < 0.0)
