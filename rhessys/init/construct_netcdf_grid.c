    ...
    struct command_line_object *command_line)
{
    ...
    for (int var = 0; var < clim_vars_counts; var++)
    {
        ...
        {
            fprintf(stderr, "Invalid or missing filename or var_name in construct_netcdf_grid.c\n");
            exit(1);
        }
        ...
        //fprintf(stdout, "Finish reading netcdf data for var %s \n", var_name);
    } // var
#ifdef LIU_EXTEND_CLIM_VAR
    for (j = 0; j < duration->day; j++)
    {
        ...
        daily_clim->relative_humidity[j] =
            (daily_clim->relative_humidity_max[j] + daily_clim->relative_humidity_min[j]) / 2.0;
    }
#endif
