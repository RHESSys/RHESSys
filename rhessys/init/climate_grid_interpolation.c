    ...
    long day)
{
    ...
    long ID_found[num_world_base_stations];
    ...
    double min_tmin = -50; // TODO, in the future put these two variable into the zone.def sfiles
    ...
    for (i = 0; i < num_world_base_stations; i++)
    {
        ...
        if (fabs(station_search[0].proj_x - zone[0].x_utm) <= search_x && fabs(station_search[0].proj_y - zone[0].y_utm) <= search_y)
        {
            ...
            if (command_line[0].verbose_flag == -3)
            {
                printf("\n there are %d neibourge stations, ID is %ld, distance is %lf, weight is %lf\n", count, ID_found[count], distance[count], weight[count]);
            }
            count++;
        }
    }
    ...
    if (count > 0)
    {
        ...
        {
            if (distance[j] > 2.0)
            {                                        // when calculate distance I using res to normalize the distance
