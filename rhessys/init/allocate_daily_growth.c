						  ...
						  struct date current_date)
{
	...
	ndf->npool_to_leafn              = (nlc / cnl) * pnow;
	ndf->npool_to_leafn_store      = (nlc / cnl) * (1.0-pnow);
	ndf->npool_to_frootn             = (f1 * nlc   / cnfr) * pnow;
	ndf->npool_to_frootn_store     = (f1 * nlc  / cnfr) * (1.0-pnow);
	if (epc.veg_type == TREE){
		ndf->npool_to_livestemn          = (nlc * f3 * f4 / cnlw) * pnow;
		ndf->npool_to_livestemn_store  = (nlc * f3 * f4 / cnlw) * (1.0-pnow);
		ndf->npool_to_deadstemn          = (nlc * f3 * (1.0-f4) / cndw) * pnow;
		ndf->npool_to_deadstemn_store  = (nlc * f3 * (1.0-f4)/cndw) * (1.0-pnow);
		ndf->npool_to_livecrootn         = (nlc * f2 * f3 * f4 / cnlw) * pnow;
		ndf->npool_to_livecrootn_store = (nlc * f2 * f3 * f4 / cnlw) * (1.0-pnow);
		ndf->npool_to_deadcrootn         = (nlc * f2 * f3 * (1.0-f4)/cndw) * pnow;
		ndf->npool_to_deadcrootn_store = (nlc * f2 *f3*(1.0-f4)/cndw)*(1.0-pnow);
	}
