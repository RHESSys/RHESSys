		...
		if (command_line[0].gw_flag > 0) {
			...
			default_object_list[i].surf_to_gw_coeff *= command_line[0].surf_to_gw_coeff_mult;
				...
				default_object_list[i].sat_to_gw_coeff *= command_line[0].sat_to_gw_coeff_mult;
					...
					default_object_list[i].rain_threshold = getDoubleParam(&paramCnt, &paramPtr, "rain_threshold", "%lf", 0.0, 1); //default will always route>0
					printf("\n precipitation threshold is %lf", default_object_list[i].rain_threshold);
			}
