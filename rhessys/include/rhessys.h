...
/* build a beetle caused snag pool sequences based on clim_event_sequence N.REN 20180630*/

struct dated_sequence2
        {
         struct date edate;
         double Cvalue;
         double Nvalue;
...
struct snag_sequence_object
        {
         int inx; // the index of climate input
         struct dated_sequence2 *seq;
...
struct  dated_input_object
        ...
        struct clim_event_sequence CO2; // CO2 concentration daily time serires ppm  Ning Ren 20181026
        };
...
/*----------------------------------------------------------*/
        ...
        //double  *surface_shortwave_rad;         /*      W m-2  daily average  *///remove for pronghorn
...
struct spinup_default {
        ...
        double min_years; /* minmum years required to reach target */
...
struct epvar_struct
{
    ...
    double all_pai_when_red;      /* the all_pai for calculating the interception of snow and radiation*/
    double proj_pai;       /* (DIM) projected plant area index */
    double proj_pai_when_red;     /* the pai for calculating the interception of snow and radiation when beetle attack. NREN 20180803 */
    double all_lai;        /* (DIM) live all-sided leaf area index */
    double proj_lai;       /* (DIM) live projected leaf area index */
    double proj_lai_when_red;     /* the lai for calculating the interception of snow and radiation when beetle attack, the EVP stopped but the leaf on N.REN 20180727 */
 ...
 struct nstate_struct
{
    ...
    double snagn; //beetle snag pool this is the ready to decay pool
...
struct  canopy_strata_object
	...
	struct  fire_effects_object fe;
        ...
        struct  mult_conduct_struct     mult_conductance;
                ...
                /* add the snag from beetle attack events N.REN 20180715*/
        struct snag_sequence_object snag_sequence; //not a structure or union error, not use pointer
...
struct beetle_default {
     ...
     double year_delay; //snag pool delay
     double half_life;
     int n_rows;
     int n_cols;
     int beetle_in_buffer;
     int year_attack;
     double leaf_year_delay; //red needle delay uear
     double leaf_half_life;  // red needle half life
