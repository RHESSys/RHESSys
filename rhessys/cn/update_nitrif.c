				  ...
				  double std)
{
	...
	if ((ns_soil->sminn + patch[0].sat_NH4) > ZERO ) {// && patch[0].totalc > ZERO eventually I didn't use resource satNH4, here is why N is accumulating in no-veg patches
      ...
      /*  if (patch[0].rain_throughfall > 0.002)
       ...
       nitrify_total = min(ns_soil->sminn + patch[0].sat_NH4, water_scalar * T_scalar * N_scalar_total * pH_scalar * max_nit_rate) ;
       ...
       // nitrify_soil = min(ns_soil->sminn, water_scalar * T_scalar * N_scalar_soil * pH_scalar * MAX_RATE * (ns_soil->sminn )); // ns_soil->sminn
        nitrify_soil = min(ns_soil->sminn, water_scalar * T_scalar * N_scalar_soil * pH_scalar * max_nit_rate ); // ns_soil->sminn
       ...
       if (ns_soil->sminn + patch[0].sat_NH4 <= ZERO) {
        ...
        if(nitrify_soil + nitrify_sat - nitrify_total < ZERO)
        {
       ...
       /* if(nitrify_total!=nitrify_total || isinf(nitrify_total) || nitrify_total<0 || nitrify_soil<0 || nitrify_sat<0 || patch[0].sat_NH4<0){
        ...
        //printf("nitrification checking [ID %d],[nitrify_soil, %f],[nitrify_sat , %f], [rain_f, %lf]\n", patch[0].ID, nitrify_soil, nitrify_sat, rain_factor);
