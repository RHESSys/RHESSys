					 ...
					 FILE *outfile)
{
	...
	asat_deficit_z = 0.0 ;
	asat_deficit = 0.0 ;
	arz_storage = 0.0;		/* Taehee Hwang */
	aunsat_storage = 0.0 ;
	arz_drainage = 0.0;		/* Taehee Hwang */	
	aunsat_drainage = 0.0 ;
	acap_rise = 0.0 ;
	areturn_flow = 0.0 ;
	aevaporation = 0.0 ;
	...
	for (h=0; h < basin[0].num_hillslopes; h++){
		...
		for (z=0; z< hillslope[0].num_zones; z++){
			...
			for (p=0; p< zone[0].num_patches; p++){
				...
				arz_storage += patch[0].rz_storage * patch[0].area;		/* Taehee Hwang */
				aunsat_storage += patch[0].unsat_storage * patch[0].area;
				arz_drainage += patch[0].rz_drainage * patch[0].area;		/* Taehee Hwang */
				...
				asublimation += patch[0].evaporation * patch[0].area;
	...
	fprintf(outfile,"%d %d %d %d %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf \n",
		date.day,
		date.month,
		date.year,
		basin[0].ID,
