					 ...
					 int verbose_flag)
{
 ...
 Nout = compute_Nsat_leached(
	...
	for (j = 0; j < patch[0].innundation_list[d].num_neighbours; j++) {
		...
		/*if (neigh[0].theta > patch[0].soil_defaults[0][0].diff_th &&
            //neigh[0].canopy_strata[0][0].defaults[0][0].epc.hot_spot !=1 &&                                    //wet period, MSR is off MSR_on = 0, default MSR_on =1, so default diff_th=1 to make it on by default
            patch[0].canopy_strata[0][0].defaults[0][0].epc.hot_spot == 1)
            {
                patch[0].MSR_on = 0;
                printf("\n MSR off [sm %lf], [neighID %d] ", neigh[0].theta, neigh[0].ID);
            }
        else {
                 patch[0].MSR_on = 1;
            } */
