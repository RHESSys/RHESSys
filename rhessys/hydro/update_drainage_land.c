					 ...
					 int verbose_flag)
{
	...
	if ( route_to_patch > available_sat_water)
		...
		if (  patch[0].rain_throughfall < patch[0].soil_defaults[0][0].rain_threshold && patch[0].soil_defaults[0][0].water_film == 1 && patch[0].canopy_strata[0][0].defaults[0][0].epc.veg_type == NON_VEG) { // &&
