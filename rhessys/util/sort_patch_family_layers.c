...
void sort_patch_family_layers(struct patch_family_object *patch_family, int *rec)
{
	...
	for (i = 0; i < patch_family[0].num_layers; i++)
	{
		...
		for (j = 0; j < patch_family[0].num_canopy_strata; j++)
		{
			...
			/*--------------------------------------------------------------*/
			if (patch_family[0].canopy_strata[j][0].epv.height == (patch_family[0].layers[i]).height)
			{
				/*--------------------------------------------------------------*/
		...
		/* Cleanup note: cover_fraction is retained for diagnostics and intentionally unused. */
		//(void)cover_fraction;
	}
	return;
}
